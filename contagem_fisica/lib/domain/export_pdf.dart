...
class GeradorPdf {
  ...
  Future<File> gerar({
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          pw.Paragraph(text: 'Operador: ${sessao.operadorNome} (${sessao.operadorMatricula})'),
          pw.Paragraph(text: 'Início: ${_fmt.format(sessao.dataInicio)}'),
          pw.Paragraph(
              text: 'Fim: ${sessao.dataFimReal != null ? _fmt.format(sessao.dataFimReal!) : '-'}'),
          pw.SizedBox(height: 12),
          pw.TableHelper.fromTextArray(
            headerAlignment: pw.Alignment.centerLeft,
            cellAlignment: pw.Alignment.centerLeft,
            headerStyle: pw.TextStyle(fontWeight: pw.FontWeight.bold),
            headers: ['Fornecedor', 'Material', 'Anterior', 'Receb.', 'Contado', 'Status'],
            data: [
              for (final it in itens)
                [
                  matByCodigo[it.materialCodigo]?.fornecedor ?? '',
                  matByCodigo[it.materialCodigo]?.descricao ?? it.materialCodigo,
          ...
          ...itens
              .where((i) => i.justificativa != null && i.justificativa!.trim().isNotEmpty)
              .map((i) => pw.Paragraph(
                    text:
    ...
    final nome = 'auditoria_${sessao.id}_${DateTime.now().millisecondsSinceEpoch}.pdf';
